...
#endif
            psz_type = "video";
            if (s->r_frame_rate.num) {
                fmt.video.i_frame_rate = s->r_frame_rate.num;
                fmt.video.i_frame_rate_base = s->r_frame_rate.den * __MAX( cc->ticks_per_frame, 1 );
            } else if(s->time_base.num*1000LL > s->time_base.den) {
                fmt.video.i_frame_rate = s->time_base.den;
                fmt.video.i_frame_rate_base = s->time_base.num * __MAX( cc->ticks_per_frame, 1 );
            }else if(cc->time_base.num*1000LL > cc->time_base.den){
